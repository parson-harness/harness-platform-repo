...
output "cd_webhook_trigger_id" {
  ...
  value       = var.create_ci_completion_trigger && var.create_standard_ci_gradle && var.create_strategy_pipeline ? harness_platform_triggers.cd_webhook_trigger[0].id : null
}

output "cd_webhook_trigger_identifier" {
  description = "Identifier of the CD webhook trigger for auto-deploy"
  value       = var.create_ci_completion_trigger && var.create_standard_ci_gradle && var.create_strategy_pipeline ? harness_platform_triggers.cd_webhook_trigger[0].identifier : null
}
